    ...
    <section id="top" className="hero-stage relative flex min-h-screen flex-col justify-center px-6 pt-28 pb-20">
      ...
      <div className="mx-auto grid w-full max-w-6xl items-center gap-14 lg:grid-cols-[1.08fr_0.92fr]">
        <div>
          ...
          <motion.p initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className="hero-description">I help teams turn complex ideas into reliable AI, full-stack, and data-driven products—clearer workflows for users, stronger systems for the people who run them. I use AI to move faster where it creates leverage, while owning the architecture, code, quality, and delivery.</motion.p>
          <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="hero-target-line">open to junior · associate software, AI, GenAI, Python & full-stack roles</motion.p><motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }} className="hero-tags">{["AI products", "Full-stack systems", "Production-minded"].map((item) => <span key={item}>{item}</span>)}</motion.div>
