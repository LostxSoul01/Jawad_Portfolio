...
const caseStudySlugs = projects.filter((project) => project.caseStudySlug).map((project) => project.slug);
...
export default async function ProjectCaseStudy({ params }: PageProps) {
  const { slug } = await params;
  const project = projects.find((item) => item.slug === slug && item.caseStudySlug);
    ...
    <main className="min-h-screen px-6 py-10 sm:py-16">
      <div className="mx-auto max-w-5xl">
        ...
        <section className="mt-16 grid gap-10 lg:grid-cols-[0.8fr_1.2fr]">
          ...
          <div className="space-y-4">{study.decisions.map((decision) => <div key={decision} className="flex gap-3 rounded-xl border border-hairline bg-surface/60 p-4 text-sm leading-relaxed text-text-muted"><CheckCircle2 size={17} className="mt-0.5 shrink-0 text-signal" />{decision}</div>)}</div>
        </section>
        ...
        <section className="mt-16 grid gap-6 sm:grid-cols-2">
          ...
          <div className="case-study-panel"><p className="case-study-kicker">Outcome</p><p className="mt-5 leading-relaxed text-text-muted">{study.outcome}</p><p className="mt-8 font-mono text-xs uppercase tracking-widest text-text-faint">Stack</p><div className="mt-4 flex flex-wrap gap-2">{project.tech.map((technology) => <span key={technology} className="rounded-full border border-hairline-bright px-3 py-1.5 font-mono text-xs text-text-muted">{technology}</span>)}</div>{project.metric && <p className="mt-8 border-l-2 border-signal pl-4 font-mono text-sm leading-relaxed text-text-primary">{project.metric}</p>}</div>
        </section>
